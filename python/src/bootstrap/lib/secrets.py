...
entry it extracts the bootstrap age key from 1Password (the single secret-
zero reference) into a mode-0600 file under `$XDG_RUNTIME_DIR/bootstrap/`,
uses that key to decrypt the bootstrap repo's own sops-encrypted secrets
file (bundled as package data via hatchling `force-include`), and populates
...
async def _decrypt_sops_secrets(age_key_file: Path) -> dict[str, str]:
...
async def ephemeral_secrets(ctx: Context) -> AsyncIterator[None]:
    ...
    populating the context fields. Consumers gate real work on
    `ctx.dry_run` and never read the empty fields in that branch, so the
    type signature stays honest: `Path | None` means "real path or not
    populated" with no magic sentinel values.
    """
    if ctx.dry_run:
    ...
    age_file.write_text(await op.read(OP_AGE_KEY_PATH) + "\n")
    age_file.chmod(stat.S_IRUSR | stat.S_IWUSR)

    try:
        secrets_fields = await _decrypt_sops_secrets(age_file)
        try:
